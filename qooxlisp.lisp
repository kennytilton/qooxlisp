        ...
        (b-if self (b-if oid (parse-integer (req-val req "oid") :junk-allowed t)
          ...
          (let ((opcode (qxl-sym (req-val req "opcode"))))
            (b-if cb (funcall opcode self)
              (funcall cb self req)
              (warn "Widget ~a oid ~a in session ~a has no handler for ~a callback " self (oid self) (session-id session) opcode)))
          (warn "Widget not found for oid ~a in session ~a for ~a callback" (oid self) (session-id session) (req-val req "opcode")))
        (warn "Unknown session ID ~a in callback: ~a" (req-val req "sessId") 
          (list (req-val req "opcode") (req-val req "oid")))))))
